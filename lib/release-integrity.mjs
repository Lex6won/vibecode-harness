...
// This validates the approval record itself without inspecting the files in a
// folder. It is used only during an in-place update: the old manifest is kept
// outside the install directory before the new payload is extracted.
export async function readTrustedManifest({ manifestPath, trustPath }) {
  ...
  if (!verify(null, manifestPayload(manifest), createPublicKey(signer.public_key_pem), Buffer.from(manifest.signature, "base64"))) {
    throw new Error("The approved bundle manifest signature is invalid.");
  }
  return { manifest, signer_key_id: signer.key_id };
}

function safeBundlePath(root, manifestPath) {
  const target = resolve(root, ...String(manifestPath).split("/"));
  if (normalizedRelative(root, target) !== manifestPath) throw new Error("A manifest path escapes the bundle directory.");
  return target;
}
...
async function removeEmptyParents(root, path) {
  ...
  while (current !== root) {
    try {
      ...
      // A directory containing a current or user file must remain in place.
      break;
    }
    current = dirname(current);
  }
...
export async function verifyBundle({ bundleDir, trustPath, now = new Date(), allowedExtraPaths = [] }) {
  const root = resolve(bundleDir);
  ...
  try {
    ...
    for (const path of allowedExtraPaths) {
      if (typeof path !== "string" || !path || path.includes("\\") || path.includes("..") || path.startsWith("/")) {
        throw new Error("An allowed extra bundle path is unsafe.");
      }
      safeBundlePath(root, path);
      allowedExtras.add(path);
    }
    const actualFiles = await collectFiles(root);
    const actual = [];
    for (const path of actualFiles) {
      const relativePath = normalizedRelative(root, path);
      if (!allowedExtras.has(relativePath)) actual.push({ path: relativePath, sha256: await sha256File(path), size: (await stat(path)).size });
    }
...
export async function cleanupSupersededBundleFiles({ bundleDir, previousManifestPath, trustPath, backupDir }) {
  const root = resolve(bundleDir);
  const current = await readTrustedManifest({ manifestPath: join(root, MANIFEST_NAME), trustPath });
  const previous = await readTrustedManifest({ manifestPath: previousManifestPath, trustPath });
  const currentPaths = new Set(current.manifest.files.map((file) => file.path));
  const staleFiles = previous.manifest.files.filter((file) => !currentPaths.has(file.path));

  // Before deleting anything, prove that every new payload file is valid and
  // that the only extra files are precisely files approved by the old bundle.
  const staged = await verifyBundle({
    bundleDir: root,
    trustPath,
    allowedExtraPaths: staleFiles.map((file) => file.path)
  });
  if (staged.status !== "verified") throw new Error(`Updated bundle verification failed before cleanup: ${staged.reason || staged.status}`);

  const result = {
    status: "cleaned",
    previous_bundle_id: previous.manifest.bundle_id,
    current_bundle_id: current.manifest.bundle_id,
    removed: [],
    backed_up: [],
    missing: []
  };
  const backupRoot = resolve(backupDir || join(dirname(root), ".vibecode-harness-update-backups", `cleanup-${Date.now()}`));
  ...
  for (const file of staleFiles) {
    ...
    if (!metadata.isFile()) throw new Error(`Stale bundle path is not a regular file: ${file.path}`);
    const unchanged = metadata.size === file.size && await sha256File(target) === file.sha256;
    if (!unchanged) {
      const backupPath = safeBundlePath(backupRoot, file.path);
      await mkdir(dirname(backupPath), { recursive: true });
      await copyFile(target, backupPath);
      result.backed_up.push({ path: file.path, backup_path: backupPath });
    }
    await rm(target);
  ...
  const final = await verifyBundle({ bundleDir: root, trustPath });
  if (final.status !== "verified") throw new Error(`Updated bundle verification failed after cleanup: ${final.reason || final.status}`);
  return result;
}
